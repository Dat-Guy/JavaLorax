...
public class Main {
	...
	private static final int[] PRINT_QTY_ELEMENT = new int[]{1,2,3}; //Print quantity indexs
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			Long PrintQty = (long) 0; //Grab print task quantity
			
			for (var j = 0; j < PRINT_QTY_ELEMENT.length; j++){
				PrintQty += Long.parseUnsignedInt(data.get(i).get(PRINT_QTY_ELEMENT[j]));	
			}
